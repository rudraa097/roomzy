...
import { ChevronLeft, Download, Send, RotateCcw } from 'lucide-react';
  ...
  if (generated) {
    ...
    return (
      <div className="flex h-full flex-col bg-slate-50">
        <div className="shrink-0 bg-gradient-to-br from-violet-900 to-violet-700 p-6 text-white">
        ...
        <div className="flex-1 overflow-y-auto p-4">
          ...
          <div className="flex gap-3">
            <button
              onClick={() => showToast("Agreement PDF downloading! 📥")}
              className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-violet-600 py-4 text-sm font-bold text-white shadow-lg active:scale-95"
            >
              <Download size={18} /> Download
            </button>

            <button
              onClick={() => showToast("Agreement sent! 📧")}
              className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-emerald-600 py-4 text-sm font-bold text-white shadow-lg active:scale-95"
            >
              <Send size={18} /> Send
            </button>
          </div>
    ...
    <div className="flex h-full flex-col bg-slate-50">
      ...
      <div className="flex-1 p-6">
        <div key={step} className="space-y-5">
          {s.fields.map(([l, k, t]) => (
            <div key={k}>
              <label className="mb-1.5 block text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                {l}
              </label>
              <input
                type={t}
                ...
                onChange={update(k)}
                className="w-full rounded-xl border border-slate-200 px-4 py-3.5 text-sm outline-none focus:border-violet-400"
              />
            </div>
          ))}
          ...
          <div className="flex gap-3 pt-4">
            {step > 1 && (
              <button
                ...
                className="flex-1 rounded-xl bg-slate-100 py-4 text-sm font-bold text-slate-600 active:scale-95"
              >
                ← Back
              </button>
            )}
