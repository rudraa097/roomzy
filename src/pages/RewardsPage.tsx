...
import { ChevronLeft, Copy, Check, Lock } from 'lucide-react';
...
export const RewardsPage: React.FC<RewardsPageProps> = ({ onBack, showToast }) => {
  ...
  return (
    <div className="flex h-full flex-col bg-slate-50">
      <div className="shrink-0 bg-gradient-to-br from-amber-900 to-orange-800 p-6 text-white">
        <button
          onClick={onBack}
          className="mb-4 flex items-center gap-1 text-xs font-bold uppercase tracking-wider opacity-80"
        >
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        <div className="rounded-2xl border-2 border-dashed border-amber-200 bg-amber-50 p-5">
          ...
          <p className="mb-4 text-xs text-amber-700">
            Share your code. When a friend books a room, you both get 200 points!
          </p>
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className={`flex h-14 w-14 items-center justify-center rounded-xl text-white shadow-lg transition-all active:scale-95 ${
                copied ? "bg-emerald-600" : "bg-amber-600"
              }`}
            >
              ...
              <div className="mb-1 flex items-center justify-between">
                ...
                {!p.unlocked && (
                  <div className="flex items-center gap-1 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                    <Lock size={10} /> {p.pts - myPts} more pts
                  </div>
                )}
              </div>
