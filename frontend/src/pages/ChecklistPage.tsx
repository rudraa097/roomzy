...
export const ChecklistPage: React.FC<ChecklistPageProps> = ({ onBack, showToast }) => {
  ...
  return (
    <div className="flex h-full flex-col bg-slate-50">
      <div className="shrink-0 bg-linear-to-br from-amber-900 to-amber-700 p-6 text-white">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {checked.size === totalItems && (
          <motion.div 
            ...
            className="flex flex-col items-center rounded-2xl border border-emerald-100 bg-emerald-50 p-6 text-center"
          >
            <div className="mb-2 text-4xl">🎉</div>
            <div className="text-base font-black text-emerald-800">All done! You're ready to move in!</div>
          </motion.div>
        )}
            ...
            <div className="p-2">
              {cat.items.map((item, ii) => {
                ...
                return (
                  ...
                  >
                    <div className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-md border-2 transition-all ${isDone ? "border-emerald-500 bg-emerald-500" : "border-slate-200"}`}>
                      {isDone && <CheckCircle2 size={14} className="text-white" />}
                    </div>
                    <span className={`text-sm transition-all ${isDone ? "text-slate-400 line-through" : "text-slate-700"}`}>{item}</span>
                  </button>
                );
              })}
            </div>
