...
export const ProfilePage: React.FC<ProfilePageProps> = ({ 
  ...
  const [isDepositPaid, setIsDepositPaid] = useState(() => localStorage.getItem('rz_deposit_paid') === 'true');
  const [isAutopayEnabled, setIsAutopayEnabled] = useState(() => localStorage.getItem('rz_autopay') === 'true');
  ...
  const handlePayDeposit = (method: 'gateway' | 'upi') => {
    if (method === 'gateway') {
      ...
      setTimeout(() => {
        ...
        localStorage.setItem('rz_deposit_paid', 'true');
  ...
  const toggleAutopay = () => {
    ...
    localStorage.setItem('rz_autopay', newState.toString());
  ...
  const [settledHistory, setSettledHistory] = useState<{ date: string, total: number, items: string[] }[]>(() => {
    try {
      const saved = localStorage.getItem('rz_settled_history');
  ...
  const [profile, setProfile] = useState(() => {
    try {
      const saved = localStorage.getItem('rz_user_profile');
      return saved ? JSON.parse(saved) : {
        name: "Rohan Kapoor",
  ...
  const saveProfile = () => {
    setProfile(editForm);
    localStorage.setItem('rz_user_profile', JSON.stringify(editForm));
  ...
  const settleAll = () => {
    ...
    const updatedHistory = [historyEntry, ...settledHistory].slice(0, 10);
    setSettledHistory(updatedHistory);
    localStorage.setItem('rz_settled_history', JSON.stringify(updatedHistory));
